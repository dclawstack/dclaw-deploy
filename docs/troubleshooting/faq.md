...
### How do I update DClaw Deploy?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp deploy --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Deploy without Kubernetes?

Yes, for local development:

```bash
cd dclaw-deploy/frontend && npm run dev
cd dclaw-deploy/backend && uvicorn dclaw_deploy.main:app --reload
```
