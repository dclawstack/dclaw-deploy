...
# Check events
kubectl get events -n dclaw-deploy --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-deploy deployment/dclaw-deploy-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-deploy-db-credentials -n dclaw-deploy
   ```
